...
def main() :
        ...
        while True:
                ...
                if(cars):
                    cv2.imwrite("print.jpg", original_img)
                ...
                actual_pos=[]
                for car in cars:
                   actual_pos.append([float(car.prev_positions[-1][0]),float(car.prev_positions[-1][1])])

                means = kmeans(actual_pos,4)

##                for car in cars:
##                    lane = vq(actual_pos,means)
##                    print(lane)
...
##                            quinai = (car.prev_positions[-1][0] + (maxDistX + minDistX)/100,car.prev_positions[-1][1] + (maxDistY)/100)
##                            cv2.rectangle(original_img,quinas, quinai, (0,255,0),7)]
                    cv2.circle(original_img, (int(j[0]), int(j[1])), 3, (0,0,255), 3)
